...
export function AccessibilityProvider() {
  useEffect(() => {
    ...
    // Usar uma função para construir o nome do módulo dinamicamente
    // Isso evita que o webpack analise estáticamente a importação
    const loadAxe = async () => {
      try {
        // Usar Function constructor para evitar análise estática do webpack
        // Isso garante que o módulo não seja incluído no bundle de produção
        const importAxe = new Function('return import("@axe-core/react")')
        const axeModule = await importAxe()
        
        if (axeModule?.default) {
