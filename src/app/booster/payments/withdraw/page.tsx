...
export default function BoosterWithdrawPage() {
  ...
  useEffect(() => {
    if (!authLoading && (!user || user.role !== 'BOOSTER')) {
    ...
    <div className="min-h-screen bg-black p-4 md:p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4 mb-6">
          <Link href="/booster/payments" className="text-gray-400 hover:text-white transition-colors">
            <ArrowLeft className="w-6 h-6" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-white font-orbitron">Solicitar Saque</h1>
            <p className="text-gray-400 font-rajdhani">Transfira suas comissões para sua conta</p>
          </div>
        </div>
