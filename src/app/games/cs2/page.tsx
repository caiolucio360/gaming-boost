          ...
          <div className="max-w-6xl xl:max-w-7xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
              {services.map((service, index) => {
                ...
                const cardElement = (
                  <Card
                    className={`group relative h-full bg-card border transition-all duration-500 overflow-hidden ${
                      isClickable
                  ...
                  >
                    ...
                    <CardHeader className="text-center pb-2 relative z-10">
                      ...
                      <CardDescription className="text-muted-foreground font-rajdhani text-lg mt-2 group-hover:text-foreground transition-colors duration-300" style={{ fontWeight: '400' }}>
                        {service.description}
                      </CardDescription>
                    </CardHeader>
                    ...
                    <CardContent className="pt-4 relative z-10 flex flex-col justify-between h-[calc(100%-140px)]">
                      <ul className="space-y-3 mb-8">
                        {service.features.map((feature, featureIndex) => (
                          <li 
                            key={featureIndex} 
                            className="flex items-center text-muted-foreground font-rajdhani group-hover:text-foreground transition-colors duration-300"
                          >
                      ...
                      <div className="text-center mt-auto">
                        {isClickable ? (
                          ...
                          <div className="inline-flex items-center justify-center px-6 py-3 bg-brand-gray-500/20 border border-brand-gray-500/50 rounded-lg text-muted-foreground font-rajdhani w-full" style={{ fontWeight: '500' }}>
                ...
                return isClickable ? (
                  <Link href={service.href} key={index} className="block outline-none focus:ring-2 focus:ring-brand-purple-light focus:ring-offset-2 focus:ring-offset-background rounded-xl h-full">
                    {cardElement}
                  </Link>
                ) : (
